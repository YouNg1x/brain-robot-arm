...
- `visual_search_controller` 在 `backend=piper` 且实体运动开启时，已将搜索、局部搜索和最后轨迹重捕获改为异步短段 MoveIt 规划。
- 每段默认最多移动 `0.08 rad`，规划成功后才发布到现有 `/brain_robot_grasp/arm_trajectory`；适配器仍是唯一实体关节命令出口。
- 规划前和发布前都要求 `/brain_robot_vision/filtered_points` 在 `2.5 s` 内更新且非空；点云过期时保持当前位置。
- 目标重新确认、急停、停止服务和状态切换会发布当前关节保持轨迹，覆盖未完成搜索轨迹；仿真后端仍保留原有 JointJog 搜索行为。
- 真机验证仍需在虚拟机重新编译后进行，至少检查搜索段规划诊断、`/joint_commands` 频率、目标出现时轨迹停止，以及规划场景过期时保持不动。
