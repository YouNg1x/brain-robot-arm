...
该文档定义持续 TRACK_HOLD、最后可信视觉历史重捕获、视觉阶段前进限制、深度置信度、点云碰撞和闭环微步抓取的实施顺序。颜色候选只用于检测器内部筛选，不再单独授权机械臂对齐。2026-09-21 已完成 A1--A4、B1--B3、C1、D1--D4 与 E1--E3 的源码/脚本修改，尚未在 Ubuntu ROS 2 环境构建或实机验收。
...
- 目标在 `ALIGN` 或 `GRASP_READY` 阶段丢失时，控制器会先进入 `LAST_PATH_REACQUIRE`：使用最近可信目标的 `base_link` 位置和 0.40 秒窗口速度估计预测 0.25 秒后的目标位置，再投影到当前相机，最多执行两次、每次最大 0.60 rad 的 J1/J5 重捕获。紫色只作为检测器候选筛选信息；控制器必须等待完整的正方体形状、尺寸和深度确认，确认成功后才进入 `ALIGN`，否则继续重获或局部搜索。
...
本轮已完成持续跟踪和重捕获 A 阶段的源码修改，尚待真机验收。`GRASP_READY` 现在保持目标监视；目标再次偏移时重新进入 `ALIGN`，丢失时不再按最近 J1/J5 命令固定偏移，而是使用最近 0.40 秒可信目标的 `base_link` 位置和速度估计生成 0.25 秒预测点，再投影到当前腕部相机，最多执行两步、每步不超过 0.60 rad 的 J1/J5 重捕获。候选颜色不会触发候选居中，只有完整目标确认后才允许进入 `ALIGN` 和 `GRASP_READY`。
