...
- 实机首次真正运动时确认，驱动使能后会立即执行适配器当前已经积累的视觉目标；原来的 `0.035 rad/s` 对当前相机视野过快，曾先统一降到五分之一。2026-09-20 根据最新实测需求，将当前真机速度在该基础上提高为三倍：适配器 `0.021 rad/s`，搜索速度 `0.015/0.012 rad/s`，居中上限 `0.015 rad/s`，水平/垂直居中增益同步改为 `0.00075`，水平锁定上限改为 `0.009 rad/s`。
