...
- 实机首次真正运动时确认，驱动使能后会立即执行适配器当前已经积累的视觉目标；原来的 `0.035 rad/s` 对当前相机视野过快，曾先统一降到五分之一。2026-09-20 根据最新实测需求，当前配置再次整体提高为上一版的三倍：适配器 `0.063 rad/s`，搜索速度 `0.045/0.036 rad/s`，居中上限 `0.045 rad/s`，水平/垂直居中增益同步改为 `0.00225`，水平锁定上限改为 `0.027 rad/s`。
